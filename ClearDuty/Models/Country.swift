...
struct Country: Hashable, Identifiable, Sendable {
    ...
    // ITU calling code, e.g. "+63".
    let callingCode: String

    var id: String { regionCode }

    // Maps each letter of the region code to its regional indicator symbol.
    var flag: String {
        regionCode.uppercased().unicodeScalars.reduce(into: "") { flag, scalar in
            guard let indicator = UnicodeScalar(0x1F1E6 - 0x41 + scalar.value) else { return }
    ...
    // E.164 for the wire: calling code plus digits only, no spaces or dashes.
    func e164(nationalNumber: String) -> String {
        callingCode + nationalNumber.filter(\.isNumber)
    }

    static let all: [Country] = callingCodes
        .map(Country.init(regionCode:callingCode:))
        .sorted { $0.name.localizedCompare($1.name) == .orderedAscending }

    // The device's Region setting, which is a default rather than the user's actual location.
